...
`;
  ...
  // agent/read-preferences.js
  var donorContext = null;
  frida_java_bridge_default.performNow(() => {
    frida_java_bridge_default.use("android.app.ContextImpl").getApplicationContext.implementation = function() {
      return this;
    };
    frida_java_bridge_default.use("android.app.Application").attach.overload("android.content.Context").implementation = function(context) {
      donorContext = frida_java_bridge_default.retain(context);
      frida_java_bridge_default.classFactory.loader = context.getClassLoader();
      frida_java_bridge_default.use("java.util.concurrent.CountDownLatch").$new(1).await();
    };
  });
  rpc.exports.read = (config) => new Promise((resolve, reject) => {
    frida_java_bridge_default.performNow(() => {
      ...
      try {
        ...
        if (context === null) {
          reject(new Error("CONTEXT_NOT_READY"));
          return;
        }
        stage = "bluetooth";
        const adapter = frida_java_bridge_default.use("android.bluetooth.BluetoothAdapter").getDefaultAdapter();
        if (adapter !== null && adapter.getState() !== 10) {
          reject(new Error("BLUETOOTH_NOT_OFF"));
          return;
        }
        stage = "file";
        ...
        for (const name of [
          ...
          if (!plain.contains(name)) {
            reject(new Error("KEYSET_MISSING"));
            return;
          }
        }
        stage = "keystore";
        const store = frida_java_bridge_default.use("java.security.KeyStore").getInstance("AndroidKeyStore");
        store.load(null);
        if (!store.containsAlias(config.alias)) {
          reject(new Error("MASTER_KEY_MISSING"));
          return;
        }
        stage = "classes";
        const Prefs = frida_java_bridge_default.use("androidx.security.crypto.EncryptedSharedPreferences");
        const keys = frida_java_bridge_default.use("androidx.security.crypto.EncryptedSharedPreferences$PrefKeyEncryptionScheme");
        const values = frida_java_bridge_default.use("androidx.security.crypto.EncryptedSharedPreferences$PrefValueEncryptionScheme");
        stage = "create";
        const prefs = Prefs.create.overload(
          "java.lang.String",
          "java.lang.String",
        ...
        while (encryptedNames.hasNext()) {
          ...
          const name = String(frida_java_bridge_default.cast(prefs, Prefs).decryptKey(encrypted));
          if (name.endsWith(config.fields.shared_key)) {
            prefixes.push(name.slice(0, -config.fields.shared_key.length));
          }
        }
        if (prefixes.length !== 1) {
          reject(new Error("AMBIGUOUS_KEY_RECORD"));
          return;
        }
        const result = {};
